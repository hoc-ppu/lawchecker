...
def load_xml(input_file_path: str) -> etree._ElementTree | None:
    ...
    pp_log.logger.info(f"Loading XML from <{input_file_path}>...")
    ...
    try:
        ...
        if type(xml_document) is etree._ElementTree:
            ...
            pp_log.logger.info(f"XML loaded from <{input_file_path}>")

    except TypeError:

        pp_log.logger.error("`load_xml` expects `input_file_path` to be a string.")

    except OSError:

        pp_log.logger.error(f"`load_xml` could not read file <{input_file_path}>.")

    except etree.XMLSyntaxError:

        pp_log.logger.error(f"<{input_file_path}> is not a valid XML file.")
...
def transform_xml(
    ...
    pp_log.logger.info("Transforming XML with XSLT...")
    ...
    try:
        ...
        if type(output_xml_document) is etree._XSLTResultTree:
            ...
            pp_log.logger.info("XML transformed.")

    except (etree.XSLTParseError):

        pp_log.logger.error("There was a problem transforming the XML.")
