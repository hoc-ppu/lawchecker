...
If you want use the project normally, install the python package with:
```bash
pip install .[gui]
```

Or if you want the minimal installation without the GUI:
```bash
pip install .
```

Or if you intend to change the code (or create a distributable version), install the python package in editable mode with development dependencies *and* install the node dependencies:
...
Run in powershell:
```powershell
pip install -e .[dev,gui]; npm install
```
</details>

<details>
<summary>On Mac (or Linux)</summary>
