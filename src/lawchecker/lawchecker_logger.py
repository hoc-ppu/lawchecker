...
def setup_lawchecker_logging() -> None:
    """Configure logging for standalone lawchecker application.

    Call this function when running lawchecker as a standalone app.
    When lawchecker is imported by another application, that application
    should configure the logger instead.
    """
    # Only add handlers if none exist (avoid duplicate handlers)
    if logger.handlers:
        return

    log_file_path = Path('logs', getpass.getuser(), 'LawChecker.log').resolve()
