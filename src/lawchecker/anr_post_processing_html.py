...
def fetch_eligible_members():
    ...
    try:
        ...
        return {
            member.find(
                'DisplayAs'
            ).text  # TODO: would it be better to use .findtext()?
...
def check_amendment_in_files(amendment_num, bill_title, marshal_files):
    ...
    for file_tree in marshal_files:
        namespaces = {
            k if k else 'default': v for k, v in file_tree.getroot().nsmap.items()
        }
        ...
        if tlc_elements:
            amendment_elements = file_tree.xpath(
                amendments_query, namespaces=namespaces
            )
            if amendment_elements:
                logger.debug(
                    f"Found amendment '{amendment_num}' in bill '{bill_title}'"
                )
                return True
            else:
                logger.debug(
                    f"Amendment '{amendment_num}' not found for bill '{bill_title}' in checking XML."
                )
        else:
...
def reorder_amendments(marshal_files, bill_title, amendment_groups):
    ...
    for checking_file in marshal_files:
        ...
        # Match bill-title
        bill_title_match = root.xpath(
            ".//akn:TLCConcept[@eId='varBillTitle']/@showAs", namespaces=namespaces
        )
        ...
        if normalized_bill_title != normalized_bill_title_match:
            logger.debug(
                f"Bill title '{normalized_bill_title}' does not match '{normalized_bill_title_match}' in {checking_file.docinfo.URL}"
            )
            continue

        logger.debug(
            f'Matched Bill Title: {normalized_bill_title_match} in {checking_file.docinfo.URL}'
        )
        ...
        for num in all_nums:
            logger.debug(
                f'<num>: {ET.tostring(num, pretty_print=True, encoding="unicode")}'.strip()
            )

        # Extract amendment order from checking file
        amendment_order = root.xpath(
            './/akn:num[@ukl:dnum]/text()', namespaces=namespaces
        )
        ...
        for amendment_number in amendment_order:
            if amendment_number in remaining_amendments:
                ...
                ordered_amendment_groups.append(
                    (amendment_number, remaining_amendments.pop(amendment_number))
                )
                was_reordered = True  # Mark as reordered
            else:
                logger.debug(
                    f"Amendment '{amendment_number}' not found in amendment groups."
                )

    # Always process remaining amendments
    if remaining_amendments:
        logger.debug(
            f"Adding unmatched amendments for bill '{bill_title}': {list(remaining_amendments.keys())}"
        )
    for unmatched_amendment in remaining_amendments.keys():
        ordered_amendment_groups.append(
            (unmatched_amendment, remaining_amendments[unmatched_amendment])
        )
...
def generate_html(xml_file, checking_file_paths, eligible_members):
    ...
    if paper_count > 1:
        ET.SubElement(
            main_summary_div, 'p'
        ).text = f'{paper_count} papers have added names today:'
    else:
        ET.SubElement(
            main_summary_div, 'p'
        ).text = 'Only one paper has added names today:'
    ...
    if checking_files:
        ...
        ET.SubElement(
            explanatory_text_1, 'span'
        ).text = ' indicates that names have been added already, '
        ...
        explanatory_text_2.text = (
            'The following XML files have been used for marshalling: '
        )
        ET.SubElement(
        ...
        collapsible_div = ET.SubElement(
            main_summary_div,
            'div',
            {'id': 'collapsible-xml-files', 'style': 'display: none;'},
        )
    ...
    for bill in sorted(bills):
        ...
        number_summary_count_div = ET.SubElement(
            bill_div, 'div', {'class': 'number-summary-count'}
        )
        ...
        if checking_files:
            ...
            logger.info(
                f"No valid checking files found for bill '{bill}'. Rendering amendments in original order."
            )
        ...
        if not amendment_groups:
            ...
            for amd_number, group in ordered_amendments:
                ...
                ET.SubElement(
                    amendment_div, 'div', {'class': 'bill-reminder'}
                ).text = bill
                ...
                # Render names to add
                names_to_add_div = ET.SubElement(
                    amendment_div, 'div', {'class': 'names-to-add'}
                )
                ...
                for item in group['items']:
                    ...
                    for name in matched_names.findall('name'):
                        name_text = name.text
                        name_div = ET.SubElement(
                            names_to_add_div, 'div', {'class': 'name'}
                        )
                ...
                if any(
                    item.find('.//names-to-remove/matched-names') is not None
                    for item in group['items']
                ):
                    names_to_remove_div = ET.SubElement(
                        amendment_div, 'div', {'class': 'names-to-remove'}
                    )
                    ...
                    for item in group['items']:
                        ...
                        for name in matched_names.findall('name'):
                            name_text = name.text
                            name_div = ET.SubElement(
                                names_to_remove_div, 'div', {'class': 'name'}
                            )
                            name_span = ET.SubElement(name_div, 'span')
                ...
                # Comments Section
                if group['comments']:
                    comments_div = ET.SubElement(
                        amendment_div, 'div', {'class': 'comments'}
                    )
                    ET.SubElement(
                        comments_div,
                        'h4',
                    ...
                    for comment in group['comments']:
                        ...
                        dashboard_id = (
                            parent_comments.get('dashboard-id')
                            if iselement(parent_comments)
                            else None
                        )
                        comment_url = (
                            f'https://hopuk.sharepoint.com/sites/bct-ppu/Lists/AddNames/DispForm.aspx?ID={dashboard_id}'
                            if dashboard_id
                            else None
                        )
                        ...
                        if dashboard_id:
                            ...
                            dashboard_link_a = ET.SubElement(
                                ...
                                {
                                    'href': comment_url,
                                    'style': 'text-decoration:none;color:#4d4d4d;',
                                },
                            )
                            ET.SubElement(
                                dashboard_link_a, 'b'
                            ).text = f'Dashboard ID: {dashboard_id}'

                        comment_p = ET.SubElement(
                            comments_div,
                            'p',
                            {
                                'title': f'Dashboard ID:{dashboard_id}'
                                if dashboard_id
                                else 'No Dashboard ID'
                            },
                        )
                        if comment.text:
                    ...
                    checkbox_div = ET.SubElement(
                        comments_div, 'div', {'class': 'check-box'}
                    )
                    checkbox_input = ET.SubElement(
...
def ticks_and_crosses(output_html_file_path, marshal_file_dir):
    ...
    if os.path.isdir(marshal_file_dir):  # TODO: change to Path
        for file_name in os.listdir(marshal_file_dir):
            if file_name.endswith('.xml'):
                try:
                    checking_files.append(
                        (file_name, ET.parse(os.path.join(marshal_file_dir, file_name)))
                    )
                except Exception as e:
    ...
    for bill_div in html_tree.xpath("//div[@class='bill']"):
        ...
        for amendment_div in bill_div.xpath(".//div[@class='amendment']"):
            amendment_number_element = amendment_div.find(
                ".//div[@class='num-info']/h2[@class='amendment-number']"
            )
            if amendment_number_element is None or not amendment_number_element.text:
                continue
            amendment_number = amendment_number_element.text.strip().replace(
                'Amendment ', ''
            )  # Remove "Amendment "

            for name_div in amendment_div.xpath(
                ".//div[@class='names-to-add']/div[@class='name']"
            ):
                name_anchor = name_div.find('.//a')
                if name_anchor is None or not name_anchor.text:
                    logger.debug(
                        f'Skipping name without anchor text in amendment {amendment_number}.'
                    )
                    continue
                ...
                logger.debug(
                    f'Checking name: {name_text} for amendment {amendment_number} in bill {bill_name}'
                )
                ...
                for file_name, checking_file in checking_files:
                    ...
                    try:
                        ...
                        if not bill_matches:
                            logger.debug(
                                f'No match for bill: {bill_name} in file: {file_name}.'
                            )
                            continue
                        ...
                        if amendment_number not in amendment_numbers:
                            logger.debug(
                                f'Amendment number {amendment_number} not found in file: {file_name}.'
                            )
                            continue
                        ...
                        for block in proposer_supporter_blocks:
                            if any(
                                ...
                                for elem in block.xpath(
                                    './/docIntroducer | .//docProponent'
                                )
                            ):
                                annotation = '✔'
                                break
                        ...
                        logger.error(
                            f'Error processing checking file: {file_name}, error: {e}'
                        )
                ...
                annotation_span = ET.SubElement(
                    ...
                    'span',
                    {
                        'class': f'green name-in-xml-indicator'
                        if annotation == '✔'
                        else 'red name-in-xml-indicator',
                        'title': 'This name appears to have been added.'
                        if annotation == '✔'
                        else 'This name does not appear to have been added.',
                    },
                )
    ...
    try:
        with open(output_html_file_path, 'wb') as output_file:
            output_file.write(
                ET.tostring(
                    html_tree, pretty_print=True, method='html', encoding='utf-8'
                )
            )
    except Exception as e:
...
def main(template_path, xml_file_path, marshal_file_dir, output_html_file_path):
    ...
    try:
        ...
        summary_content, bill_content = generate_html(
            xml_file_path, marshal_file_dir, eligible_members
        )
...
if __name__ == '__main__':
    if len(sys.argv) != 5:
        print(
            'Usage: python anr_post_processing_html.py <template_path> <xml_file_path> <marshal_file_dir> <output_html_file_path>'
        )
        sys.exit(1)
